...
def one_1():
    ...
    for row in rows:
        event = {
            ...
            'Event_Start_Time': str(row[5]),
            'Event_End_Time': str(row[6]),
            'Event_Venue': row[7],
            'Event_Picture': row[8]
        }
        events.append(event)
...
def upload_event():
    ...
    try:
        ...
        cursor.execute("""
            ...
            Event_End_Time, Event_Venue, Event_Ticket, Event_url, Event_Details, Event_Status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            event_data['Event_Id'],
    ...
    if request.method == 'GET':
        if event_id is None:
            ...
            if row:
                ...
                return jsonify({'message': 'Event found', "event_picture": event_picture}) # send_file(event_picture, mimetype='image/jpeg')
            else:
                return jsonify({'error': 'Event not found'})
    elif request.method == 'PUT':
...
def login_page():
    if request.method == 'POST':
        ...
        passcode = login_data.get('Passcode')

        cursor = cnxn.cursor()
        cursor.execute("SELECT * FROM Users WHERE User_ID = ? AND Password = ?", (user_id, passcode))
